...
const EditPassage = ({ props }: { props: EditPassageProps }) => {
    ...
    const [frequencies, setFrequencies] = useState<
        { freqLabel: string; freqValue: number }[]
    >([]);
    ...
    const populateVerses = (psg: Passage, callback: Function) => {
        ...
        const override = props.overrides.find((o) => o.passageId === psg.passageId);
        if (override) {
            ...
            (async () => {
                const locPsg = await bibleService.getPassageText(
                    user,
                    translation,
                    psg.bookName,
                    psg.chapter,
    ...
    const submitChanges = () => {
        ...
        bibleService
            .updatePassage(
                ...
                updateParam.passage,
                updateParam.newText,
                updateParam.passageRefAppendLetter
            )
            .then((resp) => {
                if (resp === 'success') {
                                ...
                                <Form.Group>
                                    ...
                                    >
                                        ...
                                        {Array(maxVerse)
                                            ...
                                            .map((_n, i) => i + 1)
                                            .map((num) => (
                                                <option key={'start-' + num} value={num}>
                                                    {num}
                                                </option>
                                            ))}
                                    </Form.Select>
                                ...
                                <Form.Group>
                                    ...
                                    >
                                        ...
                                        {Array(maxVerse)
                                            .fill(undefined)
                                            .map((_n, i) => i + 1)
                                            .filter((num) => num >= startVerse)
                                            .map((num) => (
                                                <option key={'end-' + num} value={num}>
                                                    {num}
                                                </option>
                                            ))}
                                    </Form.Select>
