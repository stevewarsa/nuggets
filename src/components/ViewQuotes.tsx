...
const navigate = useNavigate();
  ...
  const handleToolbarClick = async (direction: string) => {
    ...
    try {
      const quoteText = await bibleService.getQuoteText(user, quotes[newIndex].quoteId);
      setCurrentQuote({
        ...quotes[newIndex],
        quoteTx: quoteText
      });
      ...
      >
        ...
        <Modal.Body className="bg-dark text-white">
          {topicsLoading ? (
            ...
            <>
              <p className="mb-3">
                Select one or more topics to filter quotes. Only quotes with at least one of the selected topics will be shown.
              </p>
